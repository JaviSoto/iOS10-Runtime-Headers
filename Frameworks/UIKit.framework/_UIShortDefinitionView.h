/* Generated by RuntimeBrowser.
   Image: /Applications/Xcode5.app/Contents/Developer/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator7.0.sdk/System/Library/Frameworks/UIKit.framework/UIKit
 */

@class <_UIShortDefinitionViewDelegate>, UILabel, UIView, UITextView, _UIDefinitionValue;

@interface _UIShortDefinitionView : UIView  {
    _UIDefinitionValue *_definitionValue;
    UILabel *_localizedDictionaryLabel;
    UITextView *_definitionTextView;
    UIView *_lineView;
    <_UIShortDefinitionViewDelegate> *_delegate;
}

@property(retain) _UIDefinitionValue * definitionValue;
@property <_UIShortDefinitionViewDelegate> * delegate;


- (id)initWithFrame:(struct CGRect { struct CGPoint { float x_1_1_1; float x_1_1_2; } x1; struct CGSize { float x_2_1_1; float x_2_1_2; } x2; })arg1;
- (void)setDelegate:(id)arg1;
- (void)dealloc;
- (id)delegate;
- (id)definitionValue;
- (void)setDefinitionValue:(id)arg1;

@end
